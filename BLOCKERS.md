...
### B44 — Pre-existing apps/utility TS errors (3) survive Ch.6 build — fix-on-emit hack masks them `CLOSED` `P2`
...
**Status.** CLOSED 2026-05-27 — fixed as part of B45 diagnostic mini-session.
**Fix.** Added `@c-suite/stub-harness: workspace:*` to `apps/utility/package.json` (resolves line 15). Fixed line 121 cast to `RunState & { kind: 'synthesizer' }`. Fixed line 151 by replacing ternary with explicit if/else branches so TypeScript narrows each union variant. Removed `build:soft` from `apps/main` and `apps/utility`. Rebuilt `packages/shared-types` to pick up `topic` field in `writeback.proposed` (stale dist caused writeback-engine line 161 error). All three packages now typecheck exit-0.
**Owner.** CLOSED.

### B45 — Utility process crash-loops under `pnpm dev` `MITIGATED` `P1`
**What.** `pnpm dev` launched main + DB + migrations + vault watcher cleanly but utility forked by supervisor crashed within ~150ms; 6x restart then halt.
**Root cause (confirmed via instrumentation added in this session).** Two sequential bugs — not better-sqlite3 ABI:
1. `@c-suite/stub-harness` not declared in `apps/utility/package.json` — pnpm did not link it, causing ESM `ERR_MODULE_NOT_FOUND` for `@c-suite/stub-harness/stub` in compiled `run-loop.js`. Fix: add dep.
2. (surfaced after fix 1 via buffered stderr instrumentation) `initSqlProxy` called `port.addEventListener('message', ...)` but the port from `process.parentPort.once('message')` in an Electron utility process is `MessagePortMain` (NodeEventEmitter), not Web API `MessagePort`. `addEventListener` does not exist. Fix: replace with `.on('message', ...)` throughout; introduce `IpcPort` structural interface in `sql/proxy.ts`.
**Fix.** See build-log 2026-05-27 B45 entries.
**Verification.** `pnpm dev` (Electron 33.4.11): one `start` row in `process_events`, zero `crash` rows after 40s; clean `utility process exited cleanly` on shutdown. UTILITY_DIAG confirms `nodeVersion: v20.18.3, modulesAbi: 130, electronVersion: 33.4.11`.
...
### B47 — Production runtime relies on the stub harness; no real model client wired `NEW` `P0`
**What.** The C-Suite agent reasoning engine has NO real Claude model client. The Ch.0-4 "stub-model harness" (`packages/stub-harness`) became the de-facto production implementation because the real-client wiring was perpetually deferred ("Ch.3 wires the real SDK") and never landed. `stub.ts:27-30` THROWS on `STUB_MODE=live`; default `replay` returns canned fixtures. `dispatch.ts:17,75-98` + `run-loop.ts:20,200-248` route ALL lens/Verifier/Synthesizer/Handoff inference through `StubClaudeClient`. No `@anthropic-ai/sdk` or Claude Agent SDK dependency exists in any package.json. Additionally: playbooks hardcode rigor scores (pre-mortem:159, stakeholder-1-1:204, open-qa:121), the Synthesizer write-back proposals are empty stubs (run-loop:248, open-qa:132), the flagship cash-lever playbook fabricates tool-call data (cash-lever:37 — hardcoded array with synthetic tool_call_id), and scheduled jobs (sundayRenewal, mondayTripwire) are not wired to real MCP sources. Full evidence: `docs/reviews/mock-reliance-audit-2026-05-28.md`.
**Why it slipped.** Same root cause as B46 — chapter audits validated against the stub harness ("zero live inference in CI") and never required a live-inference + live-data end-to-end run. Surfaced by a Russell-initiated mock-reliance audit, 2026-05-28.
**Bites at.** Everything — the product cannot perform real strategic analysis, real rigor scoring, real write-back proposals, or real data citations in production. Larger than B46.
**Mitigation.** Phase 3 (real-inference wiring): add a `RealClaudeClient` behind the existing stub interface; make dispatch select real-vs-stub by env (stub = test/dev only); wire real Verifier + Synthesizer; replace fabricated playbook data with real `ctx.deps.*` MCP calls; wire scheduled jobs. Acceptance = a STUB_MODE=live INTEGRATION PROOF run with real inference + real MCP data. Awaiting Russell scope decision (likely its own session). See the audit doc remediation plan.
**Owner.** Russell scope decision pending (2026-05-28).
