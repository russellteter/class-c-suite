...
**globalShortcut.** Source: `https://electronjs.org/docs/latest/api/global-shortcut`. Key finding: `register()` returns `false` silently if the accelerator is already taken by another app — no exception thrown. C-Suite must call `isRegistered()` after `register()` and fall back to a user-configurable alternative if the default is taken. `Cmd+Shift+C` is bound to Finder's "Computer" view at the application menu level (application-scoped, not a system-wide accelerator), so `globalShortcut.register` would likely succeed — but other apps (e.g., VSCode: format document) also use it. The registration failure pattern is a P3 risk regardless of which default shortcut is chosen.
