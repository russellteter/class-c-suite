...
|---|---|---|
| **pnpm** | **10.x** — pinned at `10.33.4` [verified 2026-05-27] | Latest 10.x is 10.33.4 per `npm view pnpm@10 version`. Workspace protocol stable. Pin via `packageManager: "pnpm@10.33.4"` in root `package.json`. |
...
| **Electron** | **42.x** — pinned at `42.3.0` [verified 2026-05-27] | `npm view electron version` = 42.3.0. Latest stable. `utilityProcess.fork()` stable and current (R2 §Area 2). |
| **electron-builder** | **26.x** — pinned at `26.8.1` [verified 2026-05-27] | `npm view electron-builder version` = 26.8.1. Native `notarize` boolean + `mac.hardenedRuntime: true` + `notarytool` (not `altool`, removed Nov 2023) confirmed. |
...
| **vitest** | **3.x** — pinned at `3.2.4` [verified 2026-05-27] | `npm view vitest@3 version` latest = 3.2.4. v4.1.7 exists but ADR pins 3.x (stable, Vite ecosystem proven). Re-evaluate at Ch.12. |
| **Playwright** | **1.x** — pinned at `1.60.0` [verified 2026-05-27] | `npm view playwright@1 version` latest = 1.60.0 (≥1.50 per ADR minimum). E2E over Electron renderer (delivery.md L138). Electron-playwright peer compat: no pinned Electron peer dep in playwright@1; compat verified by pinned Electron 42 major. |
...
| **React** | **18.x** — pinned at `18.3.1` [verified 2026-05-27] | `npm view react@18 version` latest = 18.3.1. React 19 exists; ADR note says "bump to 19 if no friction at repo init." Retaining 18.x: Electron renderer has no SSR constraint but React 18 LTS is the conservative safe choice for Ch.0; Ch.5 UI mockup gate is the right boundary to evaluate 19. |
