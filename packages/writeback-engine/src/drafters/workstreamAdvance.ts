// packages/writeback-engine/src/drafters/workstreamAdvance.ts
// Source: docs/decisions/0008-write-backs-and-iterative-feedback.md §3.5
// Drafter for 'workstream-advance' artifact type. snake_case frontmatter convention.
...
export async function draft(
  ...
  if (!proposal.targetArtifactId) {
    isNew = true;
    artifactId = await nextAvailableId('workstream-advance', vaultRoot);
  } else {
  ...
  const merged: Record<string, unknown> = {
    ...
    id: artifactId,
    status: (proposal.proposedFrontmatterPatch['status'] ?? existingFm['status'] ?? 'YELLOW') as string,
    proposed_by: { run_id: runId, agent: 'Synthesizer' },
  };

  const tags = deriveTags('workstream', 'type/workstream', merged);
  merged['tags'] = tags;
  ...
  const proposedBody = renderWorkstreamAdvance(merged as Parameters<typeof renderWorkstreamAdvance>[0], aliasedBody);
...
function collectIds(fm: Record<string, unknown>): string[] {
  const idFields = ['depends_on', 'linked_workstreams'];
  const ids: string[] = [];
