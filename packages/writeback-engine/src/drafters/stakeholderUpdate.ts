// packages/writeback-engine/src/drafters/stakeholderUpdate.ts
// Source: docs/decisions/0008-write-backs-and-iterative-feedback.md §3.5
// Drafter for 'stakeholder-update' artifact type. kebab-case frontmatter convention.
...
export async function draft(
  ...
  if (!proposal.targetArtifactId) {
    isNew = true;
    artifactId = await nextAvailableId('stakeholder-update', vaultRoot);
  } else {
  ...
  // Stakeholders use type/stakeholder tag; no zone-specific subtags needed.
  merged['tags'] = ['type/stakeholder'];
  ...
  const proposedBody = renderStakeholderUpdate(merged as Parameters<typeof renderStakeholderUpdate>[0], aliasedBody);
...
function collectIds(fm: Record<string, unknown>): string[] {
  const idFields = ['related-positions', 'related-decisions'];
  const ids: string[] = [];
