// packages/writeback-engine/src/drafters/position.ts
// Source: docs/decisions/0008-write-backs-and-iterative-feedback.md §3.5
// Drafter for 'position' artifact type. kebab-case frontmatter convention.
...
export async function draft(
  ...
  if (!proposal.targetArtifactId) {
    isNew = true;
    artifactId = await nextAvailableId('position', vaultRoot);
  } else {
  ...
  // Merge patch into existing frontmatter.
  const merged: Record<string, unknown> = {
    ...existingFm,
    ...proposal.proposedFrontmatterPatch,
    id: artifactId,
    status: (proposal.proposedFrontmatterPatch['status'] ?? existingFm['status'] ?? 'active') as string,
    'proposed_by': { run_id: runId, agent: 'Synthesizer' },
  };

  // Derive tags.
  const tags = deriveTags('position', 'type/position', merged);
  merged['tags'] = tags;

  // Resolve wikilinks for related: field.
  ...
  // Body: prefer Synthesizer patch over existing body.
  ...
  const proposedBody = renderPosition(merged as Parameters<typeof renderPosition>[0], aliasedBody);
...
function collectIds(fm: Record<string, unknown>): string[] {
  const idFields = ['decision-this-supports', 'predictions-spawned', 'related-positions', 'supersedes', 'superseded-by'];
  const ids: string[] = [];
