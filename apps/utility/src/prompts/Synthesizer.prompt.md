...
**Collect positions.** Build `positionMetadata[]` to mirror EXACTLY the positions you cite in `memoMarkdown`: no more, no less. Pick your strongest positions and carry no more than 12 entries. If you would cite more than 12, narrow the memo body to your 12 strongest positions so the two stay in sync. Every `positionId` you reference in `memoMarkdown` MUST have a matching `positionMetadata` entry; a position cited in the memo with no entry causes a Verifier citation-resolution failure. Omit any position you do not cite in `memoMarkdown`. For each entry, carry over its `positionId`, `role`, `claim`, `isQuantitative`, `citations`, and `sourceText` from the source lens output. Do not invent new positions not found in `lensOutputs`.
...
- `positionMetadata` holds ONLY the positions you cite in `memoMarkdown`, capped at 12 entries (your strongest). Include an entry for every `positionId` you reference in the memo body, and omit every position you do not reference. The array must match the memo's cited positions exactly. It may be empty `[]` only if no lens positions were available. Carry over `positionId`, `role`, `claim`, `isQuantitative`, `citations`, and `sourceText` exactly as they appear in the source lens outputs.
