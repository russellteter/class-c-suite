...
NORTH STAR: the W30 cash trough and the next 13 weeks of cash.
...
NOTE: NetSuite payroll blind spot — payroll data is not accessible via SuiteQL. Any cash model that includes payroll must derive from the Cash Lever Model v5 spreadsheet, not from NetSuite SuiteQL output.

Return:
1. **Position** — one paragraph, the path you recommend.
2. **Top 3 risks from this lens.**
3. **What you need from CEO, CRO, CMO, Chief of Staff to validate or execute.**
