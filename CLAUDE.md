...
node tests/e2e/phase1-grounded-decision.mjs "<question>" <tag>  # C1+C2: drives the in-app box live → grounded 6-lens memo, screenshots MemoViewer. prereq: vite :5273
node tests/e2e/cowork-bundle-verify.mjs <memo-shortId>          # C4 handback: memo → "Draw up for Cowork" → preview → Send → handoffs/<slug>/{brief,memo,continue-prompt}.md. Waits for the COMPLETE bundle before close. prereq: vite :5273
node tests/e2e/standalone-brief-gen.mjs                          # Isolate generateHandoffBrief: one timed RealClaudeClient call, no UI/IPC (STUB_MODE=live, token from .env.local). ~140s healthy
```
...
- **`STUB_MODE` defaults to `live` in the forked utility** (`apps/main/src/supervisor.ts:73`, `?? 'live'`); `CLAUDE_CODE_OAUTH_TOKEN` is loaded from `apps/main/.env.local` by `loadEnv.ts`. The real app runs real inference by default — CI/vitest set `STUB_MODE=replay`. The cash_lever stub-guard / `STUBBED_SOURCES=['cash_model']` / `stubCashModelQuery` are **dead code on the interactive path**: cash_lever is excluded from `KNOWN_CH7_PLAYBOOK_IDS` (`run-loop.ts:39-41`) so it takes the Ch.5 grounding path and never reaches `runPlaybookGuarded` (`:127`). Interactive live cash_lever ships legitimately CLEAN on real xlsx-grounded data (verified 2026-06-01: 4/4 runs, `tool_calls=0`, real-model citations, zero stub strings) — it does NOT guard-refuse or stamp DEGRADED. The guard fires only via the cron `mondayTripwire.ts:168`. (Real gap: Ch.5 grounding swallows a read failure → `[]`, so a missing xlsx would ship an ungrounded memo CLEAN — see `tasks/followup-specs.md` Thread 3.)
...
- **RealClaudeClient requires JSON output for EVERY role; `maxTurns` is a role-aware ceiling:** `invoke()` `JSON.parse`s the model text (`realClaudeClient.ts:291`, fallback `extractJsonObject`) — a new role whose prompt emits raw prose throws `ClaudeOutputParseError`. New roles must emit the JSON the consumer expects (lenses/synth `{memoMarkdown}`-style; Handoff `{bodyMarkdown}`). Separately `maxTurns` (`:249`) is `Handoff:6 else 1`: lens/synth finalize in one turn, but a role whose first turn ends non-final needs headroom or the SDK throws "Reached maximum number of turns". `allowedTools:[]` keeps every role single-shot — there is **no autonomous tool loop** anywhere (data is gathered by deterministic orchestrator code, then handed to the model). Both traps bit the CoWork handback and were caught only by a live run, never tsc.
- **CoWork handback is live-proven (C4 handback half, 2026-06-01; both real decisions):** path = renderer "Draw up for Cowork" → `handoff.preview.requested` (payload carries `originPath`, NOT originId) → `handleHandoffPreviewRequested` → `generateHandoffBrief` (real model) → `handoff.preview.ready` `{runId, brief}` (App.tsx MUST merge runId into brief or Send is rejected) → Send → `handoff.send` → `writeHandoffBundle` writes `handoffs/<slug>/{brief.md, memo.md, continue-prompt.md}` (zone `handoff`, `commitVault:false` — disk-only/untracked like memos). Brief `frontmatter` is `.passthrough()` (carries origin_title + filename for the preview panel). C4's *render* half (clickable dated Sources section) is **CLOSED 2026-06-02** (commit `5d7a481`) — see next gotcha.
- **Evidence chain is LIT on the strategic path (C4 render, 2026-06-02, commit `5d7a481`):** `runStrategicGrounded` (`open-qa/index.ts`) emits one `vault.retrieve` `tool_calls` row per injected note via `insertToolCall` — `source_id=vault-N` (short indexed token, NOT slug — the badge prints it; indexed because the org corpus has two `context_bundle.md`), `result_json={path,title,date,excerpt}`, `agent_role='Retriever'`. `vaultRetriever.buildVaultRetrievalRows` + `vaultSourceId` are the single source of truth shared by the row AND the `[^vault-N]` marker `renderVaultProvenance` appends — so click→resolve can't drift. `tool-call:get` (`handlers.ts`) resolves **run-scoped** (`WHERE run_id=? AND source_id=?`): `vault-N` repeats across runs, so unscoped `.get()` surfaces a stale run's excerpt. NOTE: this is deterministic orchestrator-emitted evidence, NOT lens tool-use — lenses still run `allowedTools:[]` (interactive cash_lever still ships `tool_calls=0`; only the strategic open_qa path writes vault.retrieve rows). The Verifier serializes the audit trail into its model message and does NOT `VerifierInputSchema.parse` it (`verifier-runner.ts:124`) — so the synthetic `'Retriever'` role + string `call_id` are safe (the schema's strict `role`/`toolCallId:int` never fire live). Proven: run `0da8991c`, shipped_clean rigor 83, in-app click resolves. C4 click-any-CLAIM (inline prose citations) stays Phase 2.
- **C1/C2 strategic grounded path is the live default:** under `STUB_MODE=live`, `open-qa/index.ts` runs `runStrategicGrounded` — bypasses the decomposer (no `strategic_option` crash), forces the 6-lens set, injects `vaultRetriever.ts` top-8 current vault notes as `contextDocuments` + a dated provenance block. This is THE edge (live-vault grounding); proven on both real decisions 2026-06-01.
